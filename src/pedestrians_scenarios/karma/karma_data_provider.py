...
class KarmaDataProvider(CarlaDataProvider):
    ...
    def get_shifted_driving_lane_waypoint(location: carla.Transform) -> carla.Waypoint:
        ...
        """
        closest_lane_waypoint = KarmaDataProvider.get_closest_driving_lane_waypoint(
            location)
        shift = KarmaDataProvider.get_rng().randn()
        shifted_waypoint = closest_lane_waypoint
        try:
            if shift > 0.02:
                shifted_waypoint = closest_lane_waypoint.next_until_lane_end(shift)[
                    0]
            elif shift < -0.02:
                shifted_waypoint = closest_lane_waypoint.previous_until_lane_start(
                    -shift)[0]
        except IndexError:
            pass
